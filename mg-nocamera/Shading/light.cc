...
void Light::placeScene() {
	...
	RenderState *render = RenderState::instance();

	//cogemos la matriz de modelview
	Trfm3D *model = render->top(RenderState::modelview);

	switch(m_type)
	{
		case directional:
		//como es un vector, tras transformarlo lo normalizamos
		m_positionEye = model->transformVector(m_position);
		m_positionEye.normalize();
		break;

		case positional:
			//multiplicamos la posición por el modelview y transformamos el punto
			//no hay que normalizarlo ya que es un punto
			m_positionEye = model->transformPoint(m_position);
		break;

		case spotlight:
			//multiplicamos la posición por el modelview y transformamos el punto
			//luego, hacemos lo mismo con la dirección

			m_positionEye = model->transformPoint(m_position);
			m_spotDirectionEye = model->transformVector(m_spotDirection);
			//ya que la dirección es un vector, lo normalziamos
