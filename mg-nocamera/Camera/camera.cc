...
void PerspectiveCamera::updateProjection() {
	...
	//calculamos los valores top, bottom, right y left
	float fovyMedio = m_fovy / 2;
	m_top = m_near * tanf(fovyMedio); //t = n * tan(fovy/2)
	m_bottom = -1*m_top; //b = -t
	m_right = m_aspectRatio * m_top; //r = w/h * t
	m_left = -1*m_right; //l = -r
...
void Camera::updateFrame () {
	...
	//calculamos el vector F = (E-A)/(||E-A||)
	Vector3 EA = m_E - m_At;
	EA.normalize();
	Vector3 F = EA;

	//calculamos los vectores R, U y D
	Vector3 upNorm = m_Up;
	upNorm.normalize();
	m_R = crossVectors(upNorm, F);
...
void Camera::fly(float step) {
	...
	//fly -> se mueve en todos los ejes (x, y, z)
	//se cambian los vectores E y At
	m_E = m_E - m_D*step;
	m_At = m_At - m_D*step;
...
void Camera::walk(float step) {
	...
	//no hay desplazamiento en el eje Y; solo se cambian los ejes x y z
	//actualizamos los vectores E y At

	m_E.x() = m_E.x() - m_D.x()*step;
	m_E.z() = m_E.z() - m_D.z()*step;

	m_At.x() = m_At.x() - m_D.x()*step;
	m_At.z() = m_At.z() - m_D.z()*step;
	setViewTrfm();
}
...
int Camera::checkFrustum(const BBox *theBBox,
	...
	//miramos si la bounding box intersecta con alguno de los seis planos que forman el frustum,
	//que son left, right, top, bottom, near y far
	//para ello utilizaremos un for

	int planoi; //índice para recorrer los 6 planos 
	for (planoi=0; planoi<6; planoi++)
	{	
			//primero miramos si el plano i está fuera del frustum
			if(BBoxPlaneIntersect(theBBox, this->m_fPlanes[planoi]) == +IREJECT)
				{
					//+IREJECT -> Está fuera del frustum
					return 1;
				}
			//luego, miramos si intersecta	
			else if (BBoxPlaneIntersect(theBBox, this->m_fPlanes[planoi]) == IINTERSECT)
			{
				//IINTERSECT -> intersecta con el plano i
				return 0;
			}	
		}
	
	//si no, está dentro del frustum
	return -1;
